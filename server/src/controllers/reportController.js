...
export function getDashboardStats(req, res) {
  try {
    ...
    const vehicles = query('SELECT id, name, model, plate, status, odometer_working FROM vehicles');
    ...
    const activeSession = get("SELECT * FROM fueling_sessions WHERE status IN ('in_progress', 'draft') ORDER BY id DESC LIMIT 1")
      || get("SELECT * FROM fueling_sessions WHERE status = 'completed' ORDER BY id DESC LIMIT 1");
    ...
    if (activeSession) {
      ...
      // Only working vehicles count for pending!
    ...
    const mostEconomical = totalVehicles > 0 ? get(
    ...
    const highestSpender = totalVehicles > 0 ? get(
    ...
    const mostKmDriven = totalVehicles > 0 ? get(
    ...
    const fuelDistribution = totalVehicles > 0 ? query(
      `SELECT 
        CASE 
    ...
    return res.status(500).json({ error: 'Erro ao compilar dados do dashboard.' });
  }
}
...
export function exportSessionExcel(req, res) {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=Abastecimento-${session.code}.xlsx`);
