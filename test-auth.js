...
async function runTests() {
  ...
  assert(gerenteRow.password_hash !== 'Civam123' && gerenteRow.password_hash.startsWith('$2a$10$'), "Senha 'Civam123' NÃO está em texto puro (armazenada com hash bcrypt)");

  // --- TEST 2: Login com credenciais válidas ('gerente' / 'Civam123') ---
  let loginReq = new Request('https://gcs2.example.com/api/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: 'gerente', password: 'Civam123' })
  });
  let loginRes = await worker.fetch(loginReq, env);
  let loginData = await loginRes.json();

  assert(loginRes.status === 200, "Login com 'gerente' e 'Civam123' retorna status HTTP 200");
  ...
  let wrongPassReq = new Request('https://gcs2.example.com/api/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: 'gerente', password: 'senhaErrada123' })
  });
  ...
  let validChangeReq = new Request('https://gcs2.example.com/api/auth/change-password', {
    ...
    body: JSON.stringify({
      currentPassword: 'Civam123',
      newPassword: 'NovaSenhaCivam456',
      confirmPassword: 'NovaSenhaCivam456'
    })
  });
  ...
  let newLoginReq = new Request('https://gcs2.example.com/api/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: 'gerente', password: 'NovaSenhaCivam456' })
  });
